...
- **로컬 로그인**
  - 로그인용 아이디 중복 체크
  - 로그인 시 login.log 파일(winston)에 로그인 기록 저장
- **회원 정보 조회**
  - 내 회원 정보 조회 : JWT 토큰에 담긴 user의 id에 해당하는 사용자 정보만 조회
  - 회원 정보 조회 시 userinfo.log 파일(winston)에 조회 기록 저장 : 매일 자정에 데이터 초기화
- **회원 정보 수정**
- **회원 탈퇴**
...
#### 🔑 Category 생성, 수정, 삭제 : ADMIN 권한만 가능 <br/> 
#### 🔑 Category 조회 : 모든 회원 로그인 시 조회 가능 <br/>
<br/>
...
#### 🔑 Product 생성, 수정, 삭제 : ADMIN, MANAGER 권한 가능 
#### 🔑 Product 조회 : 모든 회원 로그인시 가능
<br/>
...
- **리뷰 삭제**
  - ADMIN 권한은 모든 리뷰 삭제 가능
  - MANAGER, CUSTOMER 권한은 자신이 작성한 리뷰만 삭제 가능
...
**1. Project Clone**
<details>
<summary>.env 파일 (포트폴리오 프로젝트라서 env 파일을 올립니다)</summary>
<div markdown="1">
